    ...
    public function create($val = '')
    {
		...
		if($validateOtp['user']->find() != null){
			...
			if($validateOtp['user']->where('expired <', date("Y-m-d H:i:s"))->where('user_id', user()->id)->find()){
				$initializeOtp = $OTP->initializeOtp($validateOtp['user']->where('user_id', user()->id)->find()[0]->id, 'delete');
				...
				$sendOtp = $OTP->initializeOtp($validateOtp['user']->where('user_id', user()->id)->find()[0]->otp, 'send');
				$sendOtp->send();
